...
class TestCode:
    def test_symbol(self):
        symbol = "2"
        mycode = MyCode(symbol_table=SymbolTable())

        assert mycode.symbol(symbol) == "000000000000010"

    def test_dest(self):
        dest_symbol = "M"
        assert MyCode.dest(dest_symbol) == "001"

        dest_symbol = "D"
        assert MyCode.dest(dest_symbol) == "010"

    def test_comp(self):
        comp_symbol = "A"
        assert MyCode.comp(comp_symbol) == "0110000"

        comp_symbol = "D+A"
        assert MyCode.comp(comp_symbol) == "0000010"

        comp_symbol = "D"
        assert MyCode.comp(comp_symbol) == "0001100"

    def test_jump(self):
        jump_symbol = None
        assert MyCode.jump(jump_symbol) == "000"
